...
parser.add_argument("-i", "--input", dest="input", action="store", type=str, default="output.yaml")
parser.add_argument("-o", "--out", dest="output", action="store", type=str, default="result")
parser.add_argument("-g", "--grid", dest="grid", action="store", type=int, nargs=3, default=[50,50,200])
parser.add_argument("-e", "--energy", dest="energy", action="store", type=float, nargs=2, default=[-0.1, 0.1])
...
pos, pot = s2.get_hartree_pot_z(os.path.splitext(files[0])[0])
...
s2.get_tbtrans_ldos(os.path.join(cwd, f'{args.output}.cube'), args.energy, args.grid, **option)

os.chdir(cwd)
plt.savefig(f'{args.output}.jpg')