...
def runQtNegf(job, ch_struct, ch_size, el_structs, el_size, junc_dist, in_yaml, out_yaml, nnode, nproc):
    ...
    if job == 'model':
        ...
        qtNegf(calc, dict_elec, dict_channel, qt_dir, in_yaml, out_yaml, np=nproc)
    elif job == 'plot':         
...
def main():
    ...
    parser.add_argument('-j', '--job', default='run', choices=['run', 'model', 'fdf'], help='run qt, make model, check fdf')
    ...
    runQtNegf(args.job, args.channel_struct, args.channel_size, args.elec_struct, args.elec_size, args.junc_dist, args.inp_file, args.out_file, args.nnode, args.nproc)
