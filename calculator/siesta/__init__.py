...
from ...utils.env import siesta_default_location as default_location

from math import sin, cos, sqrt, pi

from ...utils.env import siesta_dir as sul
from ...utils.env import siesta_util_vh as sv
#
...
class Siesta(object):
    ...
    def _read_default_fdf(self):
        ...
        if not self.mode:
            ...
            rpath = ['calculator', 'siesta', 'siesta_default']
        elif self.mode == 'siesta':
            self._files.extend(self.__class__.basic_inputs)
            rpath = ['calculator', 'siesta', 'siesta_default']
        elif self.mode == 'elec':
            self._files.extend(self.__class__.basic_inputs)
            rpath = ['calculator', 'siesta', 'siesta_default', 'transmission', 'elec']
        elif self.mode == 'scatter':
            self._files.extend(self.__class__.basic_inputs)
            self._files.append('TS.fdf')
            rpath = ['calculator', 'siesta', 'siesta_default', 'transmission', 'scatter']
    ...
    def run_calculator(self, nproc, **option):
        ...
        from ...utils.env import siesta_calculator as siesta_exec
        from ...utils.env import siesta_util_tbtrans as tbtrans
...
def calc_pldos(nmesh, emin, emax, npoints, orbital_index, label = 'siesta', mpi = 0, nproc = 1):
    ...
    from ...utils.env import siesta_pyprojection as pdos
    cmd = 'python %s' % pdos
    if mpi:
...
def calc_fatband(nmesh, emin, emax, npoints, orbital_index, label = 'siesta', mpi = 0, nproc = 1):
    ...
    from ...utils.env import siesta_pyprojection as fat
    cmd = 'python %s' % fat
    if mpi:
        cmd = 'mpirun -np %i ' % nproc + cmd
    os.system(cmd)
...
def get_dos(emin, emax, npoints=1001, broad=0.05, label='siesta'):
    ...
    # Eig2DOS script
    from ...utils.env import siesta_util_location as sul
    from ...utils.env import siesta_util_dos as sud
...
def get_band(simobj, pathfile, label='siesta', rerun=0):
    ...
    # gnuband script
    from ...utils.env import siesta_util_location as sul
    from ...utils.env import siesta_util_band as sub
...
def get_ldos(cell, origin, nmesh, label='siesta'):
    ...
    # run rho2xsf
    from ...utils.env import siesta_dir as sul
    from ...utils.env import siesta_util_rho as sur
...
def get_pdos(simobj, emin, emax, by_atom=1, atom_index=[], species=[], broad=0.1, npoints=1001, label='siesta'):
    ...
    # run rho2xsf
    from ...utils.env import siesta_dir as sul
    from ...utils.env import siesta_util_pdos as sup
