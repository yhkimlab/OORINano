...
dm_q_left   = DM(dynm, mesh, weig, adjust_dm, dm_cutoff)
...
dm_q_right  = DM(dynm, mesh, weig, adjust_dm, dm_cutoff)
...
dm_q_center = DM(dynm, mesh, weig, adjust_dm, dm_cutoff)
del dynm, weig
...
for qpt in dm_q_left.get_qpts():
    job_list.append([qpt, target_freq])
...
for job in job_list2:
    ...
    # 4) get Gr, SigmaL, SigmaR 
    Gr, SigmaL, SigmaR = m_t.caroli(omega, kL00, kL11, kL01, kR00, kR11, kR01, VLC, VRC, kC, eta, return_aux=1)

    # 5) Sigma^<
    import m_post as m_p
    BE = m_p.be(m
    Sigma_up = 
