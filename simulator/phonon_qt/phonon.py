...
from ...calculator.vasp import read_poscar
...
Phonon(atoms)
    ...
    def run_phtrans(self, mode='elec', mpi=0, nproc=1):
        ...
        atomsL = read_poscar('%s/POSCAR' % self._params['left_path'])
        ...
        atomsR = read_poscar('%s/POSCAR' % self._params['right_path'])
        ...
        atomsC = read_poscar('%s/POSCAR' % self._params['center_path'])
        ...
        # 3) matrix sequence
        cal_xyzl = read_poscar('%s/POSCAR' % self._params['left_path'])
        cal_xyzl.select_all()
        cal_xyzl.sort('z')
        sequence_left   = cal_xyzl.get_serials()
        
        cal_xyzr = read_poscar('%s/POSCAR' % self._params['right_path'])
        cal_xyzr.select_all()
        cal_xyzr.sort('z')
        sequence_right  = cal_xyzr.get_serials()
        
        cal_xyzc = read_poscar('%s/POSCAR' % self._params['center_path'])
        cal_xyzc.select_all()
        cal_xyzc.sort('z')
