...
from ...utils.auxil import check_file
...
def qtNegf(calc, dict_elec, struct_channel, qt_dir, inp, outp, fdf_params, np=1, det_k=0, max_nk=10, init_nk=1, step_nk=1, opts=None, show_params=False):
    '''
    calc        instance of calculator such as Siesta
    dict_elec   dictionary of electrode {'psf': M.psf, 'fdf': [elec_left.fdf,elec_rigt.fdf]}
    struct_channel  dictionary of scatter   {'psf': [M1.psf, M2.psf], 'fdf': scatter.fdf}
    ...
    calc = calc.__class__()
    calcScattering(calc, struct_channel, qt_dir[1], inp, outp, np, fdf_scatt = fdf_scatt, show_params = show_params)
    if show_params:
        return 0
    else:
        ### 3. Post processing
...
def calcScattering(calc, struct_channel, scatter_dir, finp, foutp, np, fdf_scatt = None, show_params=False):
    ...
    shutil.copy(struct_channel['fdf'], 'Input/STRUCT.fdf')
    
    ### get psf
    for psf in struct_channel['psf']:
